...
namespace SORT {
    ...
    constexpr void selectionSort(RandomIt begin, RandomIt end, Comparator compere) {
        RandomIt min;
        while (begin!=end){
            min = begin;
            RandomIt it = begin;
    ...
    constexpr void selectionSort(Container &container, int start, int size, Comparator compere) {
        int maxIn = size - 1;
        while (maxIn != start) {
